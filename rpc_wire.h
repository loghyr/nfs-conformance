...
/*
 * rpc_put_u64 -- append a big-endian uint64 (two network-order uint32s).
 * Returns 1 on success, 0 on overflow.
 */
static inline int rpc_put_u64(uint8_t *buf, size_t bufsz, size_t *pos,
			       uint64_t val)
{
	return rpc_put_u32(buf, bufsz, pos, (uint32_t)(val >> 32)) &&
	       rpc_put_u32(buf, bufsz, pos, (uint32_t)(val & 0xFFFFFFFFu));
}

/*
 * rpc_get_u64 -- read a big-endian uint64 from buffer at *pos.
 * Returns 1 on success, 0 on underflow.
 */
static inline int rpc_get_u64(const uint8_t *buf, size_t bufsz, size_t *pos,
			       uint64_t *out)
{
	if (*pos + 8 > bufsz)
		return 0;
	uint32_t hi_be, lo_be;
	memcpy(&hi_be, buf + *pos,     4);
	memcpy(&lo_be, buf + *pos + 4, 4);
...
/*
 * rpc_put_str -- encode an XDR opaque<> string: uint32 length followed by
 * the bytes zero-padded to a 4-byte boundary.
 * Returns 1 on success, 0 on overflow.
 */
static inline int rpc_put_str(uint8_t *buf, size_t bufsz, size_t *pos,
			       const char *s, size_t slen)
{
	if (!rpc_put_u32(buf, bufsz, pos, (uint32_t)slen))
		return 0;
	if (*pos + slen > bufsz)
		return 0;
	memcpy(buf + *pos, s, slen);
	*pos += slen;
	size_t pad = (4u - (slen & 3u)) & 3u;
	if (*pos + pad > bufsz)
		return 0;
